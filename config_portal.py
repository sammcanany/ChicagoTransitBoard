...
def get_current_config():
    ...
    try:
        ...
        # Mask API keys for security (show if set, but don't expose actual values)
        metra_token = getattr(config, 'METRA_API_TOKEN', '')
        cta_token = getattr(config, 'CTA_API_KEY', '')
        
        result = {
            'wifi_ssid': getattr(config, 'WIFI_SSID', ''),
            'metra_token': '********' if metra_token else '',
            'cta_token': '********' if cta_token else '',
            'has_metra_token': bool(metra_token),
            'has_cta_token': bool(cta_token),
...
<head>
    ...
    <link rel="apple-touch-icon" href="https://cdn.jsdelivr.net/gh/sammcanany/ChicagoTransitBoard@main/web/apple-touch-icon.png">
...
def save_config(params):
    ...
    metra_token = params.get('metra_token', '')
    cta_token = params.get('cta_token', '')
    
    # If keys are masked (********), preserve existing values
    if metra_token == '********':
        try:
            import config
            metra_token = getattr(config, 'METRA_API_TOKEN', '')
...
# API Keys
METRA_API_TOKEN = "{metra_token}"
CTA_API_KEY = "{cta_token}"
